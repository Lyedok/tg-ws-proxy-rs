...
after checksum verification. IPK installation does not require an architecture
bypass.

## Upgrade safety

Before replacing `/usr/bin/tg-ws-proxy`, the installer verifies checksums,
...
restores the previous binary/config/service state.
